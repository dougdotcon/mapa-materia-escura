...
---------------------------------------------
Author: Antigravity (Elite Physicist System) -> Arqueologia Digital Addendum

CONTEXTO HISTORICO (FASE 1):
Este script representa a "Fase 1" do desenvolvimento. Aqui, nós não sabíamos ainda que Omega era 117.
O objetivo deste código era PERGUNTAR aos dados: "Qual é o valor de acoplamento (alpha) que faz a gravidade entópica funcionar?"
...
# z, H(z), error
# NOTA DE ARQUEOLOGIA: Estes dados são "independentes de modelo". Eles medem a idade diferencial
# de galáxias passivas. É a âncora mais sólida que temos para o H(z).
CC_DATA = np.array([
    [0.07, 69.0, 19.6],
    [0.12, 75.0, 2.0],
    [0.20, 72.9, 29.6],
...
SN_DATA_SYNTH = np.array([
    ...
    [0.05, 36.63, 0.12],
    [0.10, 38.21, 0.10],
    [0.30, 41.20, 0.14], # Corrigido visual
    [0.50, 42.45, 0.11],
    [0.70, 43.32, 0.12],
    [1.00, 44.25, 0.15],
    [1.50, 45.30, 0.18]
...
def solve_hubble_reactive(z_array, alpha, h0, om_b=0.049, om_l=0.69):
    ...
    EXPLICAÇÃO TEÓRICA:
    Diferente do LCDM padrão, aqui introduzimos o termo 'alpha * E * (1+z)^1.5'.
    Esse termo representa a "reação" do vácuo à expansão. 
    Se alpha fosse 0, teríamos apenas bárions e constante cosmológica.
    O MCMC vai ajustar 'alpha' para ver se o universo "gosta" desse termo extra.
    """
    h_result = []
    ...
    for z in z_array:
        ...
        # E = (-b + sqrt(b^2 - 4ac)) / 2a
        # AQUI ESTÁ A MÁGICA: A solução da quadrática nos dá a taxa de expansão modificada.
        delta = b**2 - 4*c
...
def luminosity_distance(z_helio, z_cmb, alpha, h0):
    ...
    NOTA: Precisamos integrar 1/H(z) para saber a distância da luz.
    Como H(z) agora depende de alpha, a distância também depende.
    Isso afeta o brilho das Supernovas.
    """
...
def log_prior(theta):
    alpha, h0 = theta
    # Prior ranges
    # Estamos sendo agnósticos aqui: "Não sei o valor, mas deve ser positivo".
...
def log_likelihood(theta, z_cc, h_cc, err_cc, z_sn, mu_sn, err_sn):
    ...
    # 1. Chronometers Chi2 (Dados diretos de expansão)
    model_h = solve_hubble_reactive(z_cc, alpha, h0)
    chi2_cc = np.sum(((model_h - h_cc) / err_cc)**2)
    
    # 2. Supernovae Chi2 (Dados de distância)
    # model_mu = distance_modulus(z_sn, alpha, h0)
    # chi2_sn = np.sum(((model_mu - mu_sn) / err_sn)**2)
    
    # Combination
    # NOTA: O código original pesava mais os Cronômetros Cósmicos (CC)
    # porque Supernovas tem systematic errors complexos na calibração absoluta (M).
...
def run_mcmc():
    ...
    print("--- Modulo de Arqueologia Digital: Verificando Hipótese Fase 1 ---")
    ...
    print("running chain (Simulando o Universo)...")
    ...
    with open("experimental/discovery_log_001.txt", "w") as f:
        ...
        f.write(f"ARQUEOLOGIA: Foi aqui que vimos que Alpha não é zero. O universo reage.\n")
